//
//  DataValidator.swift
//  VIP-SwiftUI
//
//  Created by Youssef Jdidi on 21/12/2020.
//

import Foundation

protocol DataValidatorProtocol {
    /// Validates an email address.
    ///
    /// - Parameter value: An email to be validated.
    /// - Throws: DataError.invalidEmail
    func validateEmail(_ value: String) throws

    /// Validates a password.
    ///
    /// - Parameter value: A password to be validated.
    /// - Throws: DataError.invalidPassword
    func validatePassword(_ value: String) throws
}

class DataValidator: DataValidatorProtocol {
    func validateEmail(_ value: String) throws {
        let emailRegEx = "(?:[a-zA-Z0-9!#$%\\&‘*+/=?\\^_`{|}~-]+(?:\\.[a-zA-Z0-9!#$%\\&'*+/=?\\^_`{|}" +
            "~-]+)*|\"(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\" +
            "x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])*\")@(?:(?:[a-z0-9](?:[a-" +
            "z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\\[(?:(?:25[0-5" +
            "]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-" +
            "9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21" +
        "-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])"

        let emailTest = NSPredicate(format: "SELF MATCHES[c] %@", emailRegEx)
        guard emailTest.evaluate(with: value) else {
            throw DataError.invalidEmail
        }
    }

    func validatePassword(_ value: String) throws {
        guard value.count >= 8  else { throw DataError.invalidPassword }
    }
}
